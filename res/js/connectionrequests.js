$(document).ready( function(){
  $.post("../connectionrequests", {tag:"getconnectionrequests"}, function(data){	
...
// 						+""+""+""
	...
	//end of for loop
	 ...
	 $('.btn').click(function(e){
			...
			var btn_entity_id = elementID.split('-')[1];//alert(e.target.id);  //alert(btn_entity_id);//getting the buttonType that was clicked
			var button = e.target.id;
 			var buttonType =  button.split('_')[0];//alert(buttonType);
			var divToHide = "request_"+btn_entity_id;
		
				$('#'+divToHide).addClass('hidden');
				if (buttonType=="con"){
					//alert("posting to connect");
						$.post("../connectionrequests",{tag:"acceptconnectionrequest", Requestor_id:btn_entity_id}, function(data){	
							
							},'json');
				}else if(buttonType=="ign") {
				//	alert("posting to ignore");					
						$.post("../connectionrequests", {tag:"ignoreconnectionrequest",Requestor_id:btn_entity_id}, function(data){	
							},'json');
				}
