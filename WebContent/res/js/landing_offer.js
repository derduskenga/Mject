$(document).ready(function(){ 
	...
	if($('#offer-post').length != 0){
		...
		//.........................currency end...................................
						...
						$.post("../mapping/offer_mapping.jsp",{tag:"save_offer",offered_amount:offered_amount,offered_currency:offered_currency,
							...
							//pre-append the post on offer-post div
								...
								if(data['redir'] != null){
									...
									alert("Material offer failed to post");
								}
							}
			...
			if(type_url_offer==undefined || type_url_offer==null || !type_url_offer.trim()){
						...
						for(var i = 0;i<arrO.length;i++){
							...
							if(arrO[i]['offer_type'] == "money"){
								...
								appendServiceOffer(s_offer_name, s_offer_country,
										...
										s_offer_type,visibility_class_name,"x","x","x");
								...
								appendMaterialOffer(m_offer_category,m_offer_name,
										...
										m_names,m_entity_id,
										m_offer_type,m_visibility_class_name,"x","x","x");
				...
				var offer_id = getUrlVars()["oi"];
				
				if(offer_id==undefined || offer_id==null || !offer_id.trim()){
					var donation_request_id = getUrlVars()["dri"];
					
					if(donation_request_id==undefined || donation_request_id==null || !donation_request_id.trim()){
						notificationViewNotFound();
					}else{
						//check if id dri is integer then request the donation post
						if(Math.floor(donation_request_id) == donation_request_id && $.isNumeric(donation_request_id)){
							//fetchDonationRequest(type_url,donation_request_id);
						}else{
							notificationViewNotFound();
						}
					}
				}else{
					if(Math.floor(offer_id) == offer_id && $.isNumeric(offer_id)){
						var t = getUrlVars()["t"];
						
						if((t==undefined || t==null || !t.trim()) && (type_url_offer=="offer" || "s_offer" || "m_offer")){
							//do a post to retrieve data from specific known tables 
							fetchSpecificOffers(type_url_offer,offer_id);
							
						}else{
							//do a post to retrieve from dynamic tables  
							fetchDynamicOffers(type_url_offer,offer_id,t);
						}
					}else{
						notificationViewNotFound();
					}
				...
				if(offer_type=="money"){
					...
					$.post("../mapping/offer_mapping.jsp",{tag:"save_application_acceptance",offer_id:offer_id,application_id:application_id,offer_owner_entity_id:offer_owner_entity_id,table_name:table_name,applicant_entity_id:applicant_entity_id},function(data){
						...
						if(data['success']==1){
							...
							if(data['redir'] != null){//let the user know that something went wrong; the pst should be visible
								$('#confirm-wait-gif').addClass("hidden");
...
//.............................................
					...
					$.post("../mapping/offer_mapping.jsp",{tag:"save_service_offer",service_name:service_name,service_countries:service_countries,
						...
						if(data['success']==1){
							...
							appendServiceOffer(service_name,service_countries,state,residence,hours_a_day,start_date,
									...
									service_offer_details,time_details,service_offer_id,entity_id,profile_pic_path,names,offer_type,
									"hidden","x","x","x");
						}else{
					...
					success: function (data) {
						if(data['success'] == 1){
							//it was a success: append the post
							  ...
							  names,entity_id,offer_type,m_visibility_class,"x","x","x");
						}else{
							    //check if it has a redirect
							    if(data['redir'] != null){
								    //reload the page
	...
	function appendMaterialOffer(material_category,item_name,no_of_items,country,state,residence,details,profile_pic_path,
		...
		names,entity_id,offer_type,class_name,is_offer_complete,apply_visibility_class,accept_visibility_class){
			$('#offer-post').prepend(""+
				"<div class='row col-xs-12 post' id='offer-post-"+material_offer_id+"-" + offer_type +  "'>" +
					...
					"<div class='row col-xs-12 offer-post-actions' id='offer-post-actions-"+material_offer_id+"-" + offer_type +  "'>" +
						...
						"<a href='#' class='offer-link " + class_name + " " + apply_visibility_class + "' id='offer-apply-link-"+material_offer_id+"-" + entity_id + "-" + offer_type + "' title='Apply to this offer'>Apply &nbsp;</a>"+
				 ...
				 if(is_offer_complete == "no" && accept_visibility_class == "hidden"){
					//add a button requesting successiful applicant to approve for the offer owner to acquire points
					appendApproveButton(material_offer_id,offer_type);
				}else if(is_offer_complete == "yes"){
					markAsApproved(material_offer_id,offer_type);
				}
	...
	function appendServiceOffer(service_name,service_countries,state,residence,hours_a_day,start_date,
				service_offer_details,time_details,service_offer_id,entity_id,profile_pic_path,names,
				offer_type,visibility_class_name,is_offer_complete,apply_visibility_class,accept_visibility_class){
					$('#offer-post').prepend(""+
						"<div class='row col-xs-12 post' id='offer-post-"+service_offer_id+"-" + offer_type + "'>" +
							...
							"<div class='row col-xs-12 offer-post-actions' id='offer-post-actions-"+service_offer_id+"-" + offer_type +  "'>" +
								...
								"<a href='#' class='offer-link " + visibility_class_name + " " + apply_visibility_class + "' id='offer-apply-link-"+service_offer_id+"-" + entity_id + "-" + offer_type +  "' title='Apply to this offer'>Apply &nbsp;</a>"+
					...
					if(is_offer_complete == "no" && accept_visibility_class == "hidden"){
						//add a button requesting successiful applicant to approve for the offer owner to acquire points
						appendApproveButton(service_offer_id,offer_type);
					}else if(is_offer_complete == "yes"){
						markAsApproved(service_offer_id,offer_type);
					}
