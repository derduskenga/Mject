...
public class Offer{
	...
	String notification_originator;
	HttpSession session;
	Logger log= null;
	...
	public JSONObject savefferPost(String offered_amount,String offered_currency,String offered_category,String offer_details,String offer_summary,String names){
		...
		try{
			...
			if(rs != null){
				...
				String type = "offer";
				notification_url = Utilities.NOTIFICATION_URL + "offer/?type=" + type + "&oi=" + rs.getString(1);
				notification_msg = "<a href='../user/?user=" + entity_id + "'>" + names + "</a> placed a money offer";
				notification_originator = entity_id;
				
				Notifications noti = new Notifications(entity_id);
		...
		try{
			...
			if(rs != null){
				  ...
				  obj.put("time_details",getFormattedTimeStamp("offer_applications","offer_application_date",rs.getInt(1),"offer_application_id"));
					...
					String type = "offer_application";
					String name_of_table = table_name;
					
					String notification_url = Utilities.NOTIFICATION_URL + "offer/?type=" + type + "&t=" + name_of_table + "&oi=" + offer_id;
					String notification_msg = "<a href='../user/?user=" + entity_id + "'>" + new GlobalDresser(entity_id).getNames() + "</a> applied to your offer";
					String notification_originator = entity_id;
				
					Notifications noti = new Notifications(entity_id);
					String [] recipients = noti.fetchOfferOwnerAsRecipient(offer_id,table_name);
					noti.saveNotification(notification_url,notification_msg,notification_originator,recipients);
			}else{
				obj.put("success",0);
				obj.put("message","An error ocured"); 
	...
	public JSONObject saveApplicationAcceptance(String offer_id,String application_id,String offer_owner_entity_id,String applicant_entity_id,String table_name){
		...
		try{
			...
			if(rs > 0){
				...
				String type = "service_material_acceptance";
				notification_url = Utilities.NOTIFICATION_URL + "offer/?=" + type + "&oi=" + offer_id + "&t=" +table_name;
				notification_msg = "<a href='../user/?user=" + offer_owner_entity_id + "'>" + new GlobalDresser(offer_owner_entity_id).getNames() + "</a>accepted your application";
				notification_originator = offer_owner_entity_id;
				
				Notifications noti = new Notifications(entity_id);
				String [] recipients = {applicant_entity_id};
				noti.saveNotification(notification_url,notification_msg,notification_originator,recipients);
				
				notifyClosedOffer("close_s_or_m_offer", offer_id, table_name,offer_owner_entity_id);
			}else{
				obj.put("success",0);
	...
	public JSONObject saveServiceOffer(String service_name,String service_countries,String state,String residence,String hours_a_day,String start_date,String service_offer_details,String names){
		DBConnection db = new DBConnection();
		Connection con = db._getConnection();
		JSONObject obj = new JSONObject();
		...
		try{
			...
			if(rs != null){
				...
				String type = "s_offer";
				notification_url = Utilities.NOTIFICATION_URL + "offer/?type=" + type + "&oi=" + rs.getString(1);
				notification_msg = "<a href='../user/?user=" + entity_id + "'>" + new GlobalDresser(entity_id).getNames() + "</a> applied for your offer";
				notification_originator = entity_id;
...
// 				   stSqlRecord.executeBatch(); 
				   ...
				   json.put("message","You have accepted the application. Thank you for offering on Harambesa platform");
					...
					String type = "money_acceptance";
					notification_url = Utilities.NOTIFICATION_URL + "offer/?type=" + type + "&oi=" + offer_id + "&t=" + table_name;
					notification_msg = "<a href='../user/?user=" + offer_owner_entity_id + "'>" + new GlobalDresser(offer_owner_entity_id).getNames() + "</a>accepted your application for a fund";
					notification_originator = offer_owner_entity_id;
					...
					//notify other applicants that this offer has been closed since another person's application was accepted
					notifyClosedOffer("close_money_offer", offer_id, table_name,offer_owner_entity_id);
	...
	public void notifyClosedOffer(String t, String offer_id, String table_name,String offer_owner_entity_id){
		String type = t;
		notification_url = Utilities.NOTIFICATION_URL + "offer/?type=" + type + "&oi=" + offer_id + "&t=" + table_name;
		notification_msg = "This offer put by <a href='../user/?user=" + offer_owner_entity_id + "'>" + new GlobalDresser(offer_owner_entity_id).getNames() +" has closed</a>";
		notification_originator = offer_owner_entity_id;

		Notifications noti = new Notifications(entity_id);
		String [] recipients = noti.fetchOfferApplicants(table_name, offer_id);
		noti.saveNotification(notification_url,notification_msg,notification_originator,recipients);
	}
