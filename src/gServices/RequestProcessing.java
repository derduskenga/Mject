		...
		private void checkCredentials(String username, String password){
			...
			try{
				...
				if(db._getConnection()!=null){ 
								//now fetch data
								String fetch_cred = "SELECT entity_password, password_salt, is_active,entity_id FROM entitys WHERE user_name='"+username+"'";
											...
											if(valid){
												...
												if(rs!=null){
														if(!rs.next()){
															...
															// updates the login time of this user
															String updateSql = "update entitys set last_login=now() where entity_id="+rs_c.getInt("entity_id")+"";
															PreparedStatement pstmt = db._getConnection().prepareStatement(updateSql);
															Integer rowsAffected = pstmt.executeUpdate();
															if(rowsAffected>0){
																log.info("last_login update was a success");
															}else{
																log.info("last_login update failed");
															}
														}
												}else{
													//rs is null
